...
class TransactionDao {
    ...
    getTransactionsByDate(phoneNumber, targetDate) {
        return new Promise((resolve, reject) => {
            ...
            this.db.all(sql, [phoneNumber, startOfDay.toISOString(), endOfDay.toISOString()], (err, rows) => {
                if (err) {
                    reject(err);
                } else {
                    resolve(rows || []);
    ...
    getTransactionsByMonth(phoneNumber, month, year = null) {
        return new Promise((resolve, reject) => {
            ...
            const targetYear = year ? (year.toString().length === 2 ? 2000 + parseInt(year) : parseInt(year)) : currentYear;
            ...
            this.db.all(sql, [phoneNumber, startDate.toISOString(), endDate.toISOString()], (err, rows) => {
                if (err) {
                    reject(err);
                } else {
                    resolve(rows || []);
